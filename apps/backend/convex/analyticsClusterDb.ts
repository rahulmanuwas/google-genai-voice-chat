...
export const getRecentConversationsWithTranscripts = internalQuery({
  args: {
    appSlug: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    return conversations
      .filter((c) => c.transcript && c.transcript.length > 10)
      .map((c) => ({
...
export const updateTopTopics = internalMutation({
  args: {
    appSlug: v.string(),
    period: v.string(),
    topTopics: v.string(),
